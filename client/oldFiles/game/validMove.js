...
class ValidMove {
  ...
  makePGN(allValidMoves, isCheck, promotionChosen) {
    let pgnString = "";
    if (this.type == "O") {
      if (this.to % 8 == 6) pgnString = "O-O";
      if (this.to % 8 == 2) pgnString = "O-O-O";
    } else {
          ...
          if (p != this)
            if (p.to == this.to)
              if (p.piece.type == this.piece.type) {
                ...
                else pgnString += this.idToPGN(true, false);
                break;
              }
        }
      }
  ...
  idToPGN(needX, needY, from) {
    ...
    if (from) {
      x = needX
        ? String.fromCharCode((this.piece.previousSquare % 8) + +97)
        : "";
      y = needY
        ? String.fromCharCode(
            8 - Math.floor(this.piece.previousSquare / 8) + 48
          )
        : "";
    } else {
