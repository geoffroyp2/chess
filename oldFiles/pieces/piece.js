class Piece {
  constructor([team, id, square, removed]) {
  ...
  getInfos() {
    ...
    return [this.team, this.id, this.coordinates.squareId, this.removed];
  }
  ...
  isCheck(pieces, loopAmount) {
    ...
    for (let i = moves.length - 1; i >= 0; i--) {
      ...
      // loop 1 time to look for checks
      if (loopAmount == 1)
        pieceCopy.calculateMoves(this.team == "W" ? "B" : "W", 0);

      if (pieceCopy.isSelfCheck) {
        moves.splice(i, 1);
      } else {
        // moves[i].resultingState = pieceCopy;
        if (pieceCopy.isCheck) {
          moves[i].isCheck = true;
        }
      }
    }
  }
  ...
  draw(highlight) {
    if (!this.removed) {
      ...
      image(
        pieceIconsHD[`${this.team + this.type}`],
        this.coordinates.x * 100 + 5,
        this.coordinates.y * 100 + 5,
        90,
        90
      );
    }
