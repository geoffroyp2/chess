class GameState {
    ...
    if (this.pSelected) {
      ...
      if (moveSelected) {
        if (moveSelected.type == "P" || moveSelected.type == "PX") {
          ...
          this.movePlayedPGN = moveSelected.makePGN(
            this.pieces.getMoves(this.playerTurn, this.movePlayed.piece.type),
            moveSelected.isCheck,
            null
          );
  ...
  nextPlayer() {
    if (this.playerTurn == "W") this.playerTurn = "B";
    else if (this.playerTurn == "B") this.playerTurn = "W";
  }
  ...
  promotion(x, y) {
    ...
    if (x >= this.promotionArea.x && x <= this.promotionArea.x + 80) {
      if (y >= this.promotionArea.y && y <= this.promotionArea.y + 80)
        ...
        y >= this.promotionArea.y + 80 &&
        y <= this.promotionArea.y + 160
      )
        promoteTo = "R";
      else if (
        y >= this.promotionArea.y + 160 &&
        y <= this.promotionArea.y + 240
      )
        promoteTo = "N";
      else if (
        y >= this.promotionArea.y + 240 &&
        y <= this.promotionArea.y + 320
      )
        promoteTo = "B";
    }
    ...
    if (promoteTo) {
      ...
      this.movePlayed = this.promotionMove.makePGN(
        this.pieces.getMoves(this.playerTurn, this.promotionMove.piece.type),
        this.promotionMove.isCheck,
        promoteTo
      );
      pieceMoved = true;
      this.nextPlayer();
