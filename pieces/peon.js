    ...
    validMoves(pieces) {
        ...
        if (this.team == "W") {
            ...
            if (this.coordinates.y == 6) {
                let otherPiece = pieces.find(elem => elem.coordinates.squareId == this.coordinates.squareId - 16);
                if (!otherPiece)
                    validMoves.push(this.coordinates.squareId - 16);
            }
            let otherPiece = pieces.find(elem => elem.coordinates.squareId == this.coordinates.squareId - 8);
            if (!otherPiece)
                validMoves.push(this.coordinates.squareId - 8);

            //CAPTURE
            if (this.coordinates.x > 0 && this.coordinates.y > 0) {
                let otherPiece = pieces.find(elem => elem.coordinates.squareId == this.coordinates.squareId - 9);
                if (otherPiece) {
            ...
            if (this.coordinates.x < 7 && this.coordinates.y > 0) {
                let otherPiece = pieces.find(elem => elem.coordinates.squareId == this.coordinates.squareId - 7);
                if (otherPiece) {
                    if (otherPiece.team != this.team) {
